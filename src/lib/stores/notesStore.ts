...
export async function createWelcomeNote(hotkeyLabel: string): Promise<NoteRecord> {
  const content = [
    ...
    '## Notes & subnotes',
    '',
    "Right-click a note (or the sidebar background) to create a note, rename, duplicate, move, copy/cut/paste it, export it to a `.txt` file, or delete it. Any note can hold subnotes - once it has one, it shows a folder icon: click it to open its own content, click the little arrow to expand or collapse its subnotes. Right-clicking *inside a note's text* is different - Copy/Cut/Paste there act on your current text selection via the OS clipboard, not the whole note.",
    '',
    '## Markdown',
    '',
    "Toggle **Markdown** at the bottom of a note to format as you type - headings, **bold**, lists, and more. Use the Markdown guide button (top right) for the full syntax. Links open in your default browser - Ctrl+Click (or a plain click on a locked note), right-click for more options, or Alt+O to open the link under the caret. **Alt+C** wraps the current selection in inline code (or starts a fresh span if nothing's selected); fenced code blocks and inline code both get real per-language syntax highlighting, matching Editor mode's colors.",
    '',
    '## Note linking',
    '',
    "Type `[[Note Title]]` in a markdown note to link another note by title - it becomes clickable the moment you type the closing `]]`. Click it (Ctrl/Cmd+click if the note's unlocked) to jump there. A title with no matching note yet shows dashed and muted - click it to create that note. Open a note's ⓘ popover to see its **Backlinks** - every note that links to it. Lightweight on purpose: links resolve by title, so renaming a note doesn't rewrite other notes' references to it - they just go unresolved - and there's no autocomplete, so type the title as you remember it.",
    '',
    '## Locking notes',
    '',
    ...
    '## Editor mode',
    '',
    "Press **Alt+E** to switch a note into Editor mode - a syntax-highlighted code editor with its own color scheme, search/replace (**Ctrl/Cmd+F**), and Tab-to-indent. It's a per-note toggle, independent of Markdown: a markdown note in Editor mode shows its highlighted source instead of the rich view - fenced code blocks and inline code get their own real highlighting there too. Language is auto-detected from content (JSON, JS/TS, CSS, HTML, XML, YAML, Markdown, Python, SQL, shell, and Cisco IOS / Huawei VRP / Nokia SR OS / ADVA network device configs) or pinned manually from the note-info popover (the ⓘ icon next to the note title).",
    '',
    'While in Editor mode, **Alt+F** formats the current selection, or the whole note if nothing is selected. Known languages (JSON, JS/TS, CSS, HTML, YAML, Markdown - including fenced code blocks inside it) get real formatting; network device configs get re-indented to their own vendor convention; anything else - including arbitrary text like a config snippet with no formatter - just gets a safe whitespace tidy-up, never reflowed or reindented.',
    '',
    '## Cloud notebooks',
    '',
    'Sign in from Settings → All databases to sync notes across your devices via a Supabase-backed notebook, or share one with a friend using a generated invite code. Local and cloud databases live side by side in that same list - switch between them with **Alt+B**. Fully optional: FlashPad works exactly as a local-only app until you sign in.',
    '',
    '## Shortcuts',
    '',
    ...
    '- **Alt+F** - Format the current selection, or the whole note (Editor mode only)',
    '- **Alt+R** - Toggle the line-number gutter (Editor mode only)',
    '- **Ctrl/Cmd+F** - Search/replace (Editor mode only)',
