...
export async function createWelcomeNote(hotkeyLabel: string): Promise<NoteRecord> {
  const content = [
    ...
    '## Editor mode',
    '',
    "Press **Alt+E** to switch a note into Editor mode - a syntax-highlighted code editor with its own color scheme, search/replace (**Ctrl/Cmd+F**), and Tab-to-indent. It's a per-note toggle, independent of Markdown: a markdown note in Editor mode shows its highlighted source instead of the rich view. Language is auto-detected from content (JSON, JS/TS, CSS, HTML, XML, YAML, Markdown, Python, SQL, shell) or pinned manually from the note-info popover (the ⓘ icon next to the note title).",
    '',
    'While in Editor mode, **Alt+F** formats the current selection, or the whole note if nothing is selected. Known languages (JSON, JS/TS, CSS, HTML, YAML, Markdown) get real formatting; anything else - including arbitrary text like a config snippet - just gets a safe whitespace tidy-up, never reflowed or reindented.',
    '',
    '## Cloud notebooks',
    '',
